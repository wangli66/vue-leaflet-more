        ...
        <div class="map-draw-wrap" :style="positionClass">
            <div class="map-draw" @click.stop @mousemove.stop>
                <div class="map-draw-item" :class="[d.type,{'active':d.type==editType}]" @click="handleDrawShape(d)"
                    v-for="(d,i) of drawList" :key="'draw-type-'+i" :title="d.title">
                </div>
            </div>
        </div>
    </div>
    ...
    export default {
        ...
        props: {
            ...
            position: {
                ...
                validator: function(value) {
                    return ['topleft', 'topright', 'bottomleft', 'bottomright'].indexOf(value) !== -1
                }
            },
            // 是否绘制多个图形
            drawShapeMore: {
            ...
            drawOptions: {
                type: [Array, Object],
                default: () => ['marker', 'circleMarker', 'rectangle', 'polygon', 'circle', 'polyline', 'delete'],
                // 校验数据格式
                validator: function(value) {
                    let b = true;
                    if (Array.isArray(value)) {
                        b = value.every(item => (typeof item == 'string' || item.type != undefined));
                    }
                    return b;
                }
                ...
                handler(newVal, oldVal) {
                    ...
                    this.drawList = arr.map(item => {
                        if (!typeArr.includes(item.type || item)) {
                            console.error(`绘制组件暂不支持"${item}"的绘制类型，请查看文档后重新定义\n支持的绘制类型有：${typeArr}`);
                            return false;
                        }
                        let obj = {};
        ...
        methods: {
            ...
            getIsMeasure(shapeInfo) {
                this.isMeasure = shapeInfo.measure != undefined ? shapeInfo.measure : this.measure;
            },
        ...
        mounted() {
            ...
            // console.log('this.lMap---', this.lMap, this.lMap.self);
